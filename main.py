...
def train_from_checkpoint(generation_limit):
    ...
    global i
    global current_generation
    filename = "neat-checkpoint-generation-"+str(current_generation-1)
    population = Checkpointer.restore_checkpoint(filename)
    sym =+1
    population.add_reporter(neat.StdOutReporter(True))
    stats = neat.StatisticsReporter()
    population.add_reporter(stats)
    checkpoint = Checkpointer(generation_interval=generation_limit, time_interval_seconds=3600, filename_prefix="neat-checkpoint-generation-")
    population.add_reporter(checkpoint)
    
    winner = population.run(fitness, generation_limit)
    
    
    with open("best-of-generation-"+str(current_generation-1), "wb") as f:
        pickle.dump(winner, f)
